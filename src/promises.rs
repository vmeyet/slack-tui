...
pub async fn track(
    ...
    let is_promise = judge_all(judge, &known, promise_asks, PROMISE_QUESTION, PROMISE_THRESHOLD).await?;
    let promises: Vec<&SearchMatch> = sent.iter().filter(|m| is_promise[&message_key(m)]).collect();
    ...
    let _ = cache.save(VERDICTS_CACHE, &is_promise.into_iter().chain(fulfilled).collect::<Verdicts>()).await;
    Ok(tracked)
}
